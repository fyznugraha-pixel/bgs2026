...
export default function TicketCard({ registration, type = "visitor" }: { registration: any; type?: "visitor" | "umkm" }) {
  const isUmkm = type === "umkm";
  const shortId = `BGS26-${registration.id.split("-")[0].toUpperCase()}-${isUmkm ? "UMKM" : "VIP"}`;

  const fields = isUmkm
    ? [
        { label: "NAMA USAHA", icon: "storefront", value: registration.businessName },
        { label: "KATEGORI USAHA", icon: "category", value: registration.category },
        { label: "PENANGGUNG JAWAB", icon: "person", value: registration.name },
        { label: "TANGGAL HADIR", icon: "calendar_month", value: registration.date },
      ]
    : [
        { label: "NAMA PESERTA", icon: "person", value: registration.name },
        { label: "ALAMAT EMAIL", icon: "mail", value: registration.email },
        { label: "LOKASI / GEDUNG", icon: "location_on", value: "Laswi Heritage" },
        { label: "TANGGAL HADIR", icon: "calendar_month", value: registration.date },
      ];
      ...
      <div className="p-5 md:p-8 md:w-[70%] flex flex-col justify-between relative bg-white bg-[radial-gradient(#cbd5e1_3px,transparent_3px)] [background-size:24px_24px]">
        <div>
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <span className="bg-bgs-red text-white px-3 py-1 rounded-lg font-black text-[10px] sm:text-xs uppercase tracking-wider comic-border border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transform -rotate-2">{isUmkm ? "E-TIKET UMKM" : "E-TIKET"}</span>
          ...
          <p className="text-gray-600 font-bold mb-4 md:mb-6 uppercase tracking-widest text-[10px] sm:text-xs">{isUmkm ? "Tiket Akses Tenant UMKM" : "Tiket Akses Eksklusif"}</p>

          <div className="grid grid-cols-2 gap-3 md:gap-4 mb-2 md:mb-4">
            {fields.map((field) => (
              <div key={field.label} className="bg-white p-3 md:p-3 rounded-xl comic-border border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
                <span className="text-bgs-red font-black text-[9px] md:text-[10px] uppercase block mb-1">{field.label}</span>
                <span className="font-bold text-black flex flex-col md:flex-row md:items-center gap-1 md:gap-2 break-words text-xs md:text-base">
                  <span className="material-symbols-outlined text-bgs-blue font-black text-sm md:text-base hidden md:block">{field.icon}</span>
                  {field.value}
                </span>
              </div>
            ))}
